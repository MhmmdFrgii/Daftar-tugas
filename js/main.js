...
class UIManager {
  ...
  handleAddTodo() {
    ...
    if (task === "") {
      this.showAlertMessage("Silahkan masukkan tugas", "error");
    } else {
  ...
  handleClearAllTodos() {
    ...
    this.showAllTodos();
    this.showAlertMessage("Semua tugas berhasil diselesaikan", "success");
  }
  ...
  displayTodos(todos) {
    ...
    if (todos.length === 0) {
      this.todosListBody.innerHTML = `<tr><td colspan="5" class="text-center">Tidak ada tugas</td></tr>`;
      return;
    }
  ...
  handleEditTodo(id) {
    ...
    if (todo) {
      ...
      const handleUpdate = () => {
        ...
        this.showAlertMessage("Tugas berhasil diupdate", "success");
        this.showAllTodos();
  ...
  handleDeleteTodo(id) {
    this.todoManager.deleteTodo(id);
    this.showAlertMessage("Tugas berhasil dihapus", "success");
    this.showAllTodos();
  }
